  ...
  try {
    ...
    if (debug.pathsExist.prismaClient?.exists) {
      try {
        ...
        debug.prismaFiles = files.filter(
          (f) =>
...
function generateRecommendation(debug: {
  ...
  if (!debug.pathsExist.prismaClient?.exists) {
    recommendations.push(
      'Prisma client directory not found - run "prisma generate"'
    );
  }
  ...
  if (!hasEngines) {
    recommendations.push(
      "No query engines found - check binary targets in schema.prisma"
    );
  }

  const hasRhelEngine = debug.engines.find((e: EngineInfo) =>
    e.name.includes("rhel-openssl-3.0.x")
  )?.exists;
  if (!hasRhelEngine) {
    recommendations.push(
      "Missing rhel-openssl-3.0.x engine - required for Vercel"
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "All engines appear to be present - check runtime configuration"
    );
  }
